...
def train(mnist):
    ...
    weights1 = tf.Variable(tf.truncated_normal([INPUT_NODE,LAYEER1_NODE],stddev=0.1))
    biases1 = tf.Variable(tf.constant(0.1,shape=[LAYEER1_NODE]))
    
    weights2 = tf.Variable(tf.truncated_normal([LAYEER1_NODE,OUTPUT_NODE],stddev=0.1))
    ...
    learning_rate = tf.train.exponential_decay(
                            LEARNING_RATE_BASE, 
                            global_step,mnist.train.num_examples,
                            LEARNING_RATE_DECAY)
